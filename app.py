...
ALL_FIELDS = ["candidate_name","email_addr","phone","current_company","current_role",
              ...
              "role_name","requirement_title","requirement_id","education"]
...
def hrguru_cv_filename(original_name, upload_folder=None):
    ...
    candidate = f"{base}_HRGURU{ext}"
    if not upload_folder:
        return candidate
    counter = 2
    while os.path.exists(os.path.join(upload_folder, candidate)):
        candidate = f"{base}_HRGURU_{counter}{ext}"
        counter += 1
    return candidate
...
def looks_like_person_name(line):
    ...
    blocked = [
        ...
        "skills", "developer", "engineer", "manager", "analyst", "consultant", "specialist",
    ...
    try:
        ...
        if not hf_result.get("_hf_limited") and not hf_result.get("_hf_error"):
            ...
            if hf_result.get("candidate_name"):
                result["candidate_name"] = hf_result["candidate_name"]
        ...
        if not result.get("candidate_name"):
            for line in lines_list[:18]:
                ...
                if looks_like_person_name(candidate):
                    result["candidate_name"] = candidate.title()
                    break
        ...
        if not result.get("current_company"):
            label_patterns = [
                r"(?:current\s+company|current\s+employer|company|employer|organization|organisation)[:\s-]*([A-Za-z0-9&.,' -]{2,60})",
            ...
            for pattern in label_patterns:
                ...
                if match and is_not_email(match.group(1)):
                    result["current_company"] = clean_cv_value(match.group(1), 45).title()
                    break
        ...
        if not result.get("current_role"):
            role_patterns = [
                r"(?:current\s+role|current\s+designation|designation|job\s+title|role|position|title)[:\s-]*([A-Za-z][^\n,]{2,60})",
                r"(?:working\s+as)[:\s-]*([A-Za-z][^\n,]{2,60})",
            ]
            for pattern in role_patterns:
                match = re.search(pattern, text, re.IGNORECASE)
                if match:
                    result["current_role"] = clean_cv_value(match.group(1), 55).title()
                    break
        ...
        if found_skills:
            ...
            result["key_skills"] = ", ".join(sorted_skills[:12])
        
        # ===== EXPERIENCE =====
        exp_patterns = [
            r"(\d+)\s*\+?\s*years?\s+(?:of\s+)?(?:experience|work)",
            r"(?:experience|total\s+experience)[:\s]*(\d+)\s*\+?\s*years?",
        ...
        for pattern in exp_patterns:
            ...
            if match:
                ...
                if years <= 40:
                    result["experience_years"] = f"{years} years"
                    break
...
def check_dup(conn, row):
    ...
    if phone and len(phone) >= 8:
        r = conn.execute(
            "SELECT id FROM candidates WHERE replace(replace(replace(phone,'+',''),'-',''),' ','') LIKE ? AND is_duplicate=0" + req_clause + " LIMIT 1",
            [f"%{phone}"] + params_prefix).fetchone()
        if r: return True, r[0], "phone"
    if email:
        r = conn.execute(
            "SELECT id FROM candidates WHERE lower(trim(email_addr))=? AND is_duplicate=0" + req_clause + " LIMIT 1",
...
def process_upload(recruiter_name, recruiter_email, role_override, sourcer_id, excel_file, cv_file_list):
    ...
    for cv in cv_file_list:
        if cv and cv.filename:
            ...
            if orig:
                saved_cvs.append({"orig": orig, "url": url, "pub_id": pub_id, "path": file_path, "parsed": parsed_cv})
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        if not req_id:
            validation_errors.append(f"Row {idx}: Requirement is required and must match an existing Requirement.")
        ...
        if req_id:
            ...
            if is_dup:
                validation_errors.append(f"Row {idx}: Duplicate candidate for this Requirement matched by {dup_why} (existing ID #{dup_id}).")
...
def make_template():
    ...
    headers = ["Requirement","Candidate Name","Email","Phone","Current Company","Current Role",
    ...
    hints = ["Existing requirement title or ID","Full name","Email address","10-digit mobile",
             "Current employer","Job title","e.g. 5 years","Comma separated","e.g. 30 days",
...
def build_daily_work_report_email(rows, sender_name="", filters_applied=False):
    today_label = date.today().strftime("%d %b %Y")
    subject = f"Daily Work Report - {today_label}"
    ...
    lines = [
        f"Daily Work Report - {today_label}",
        "",
        f"Sent by: {recruiter}",
        f"Candidates included: {len(rows)}",
        f"Selection basis: {'filtered/selected candidate list' if filters_applied else 'current candidate list'}",
        "",
    ]
    if status_counts:
    ...
    if not rows:
        ...
        for idx, row in enumerate(rows, 1):
            parts = [
                f"{idx}. {row.get('candidate_name') or '-'}",
                f"Role: {row.get('current_role') or row.get('role_name') or '-'}",
                f"Client/Requirement: {row.get('client_name') or '-'} / {row.get('requirement_title') or row.get('role_name') or '-'}",
                f"Status: {row.get('status') or '-'}",
                f"Experience: {row.get('experience_years') or '-'}",
                f"Location: {row.get('current_location') or '-'}",
                f"Email: {row.get('email_addr') or '-'}",
                f"Phone: {row.get('phone') or '-'}",
                f"Recruiter: {row.get('recruiter_name') or '-'}",
            ]
...
def candidate_cv_attachments(rows):
    ...
    for row in rows:
        ...
        if path.startswith(upload_folder + os.sep) and path not in seen_paths and os.path.exists(path):
            seen_paths.add(path)
...
def api_stats():
    ...
    total = sum(1 for r in rows if not int(r.get("is_duplicate") or 0))
    dups = sum(1 for r in rows if int(r.get("is_duplicate") or 0))
    roles = len({r.get("role_name") for r in rows if not int(r.get("is_duplicate") or 0) and r.get("role_name")})
...
def api_parse_jd_structured():
    ...
    if not jd_text and "jd_file" in request.files and request.files["jd_file"].filename:
        jd_path, _ = save_uploaded_analysis_file(request.files["jd_file"], "jd_extract")
...
def api_candidate_resume_analyze():
    ...
    try:
        cv_path, cv_name = save_uploaded_analysis_file(request.files["resume_file"], "resume_analysis")
        cv_text = extract_cv_text(cv_path)
        if not cv_text.strip():
...
def update_candidate(cid):
    ...
    allowed=["candidate_name","email_addr","phone","current_company","current_role",
             "experience_years","key_skills","notice_period","current_salary",
             "expected_salary","current_location","preferred_location","remarks","role_name","status","requirement_id","candidate_feedback"]
...
def add_requirement():
    ...
    if "Industry:" not in description or not re.search(r"(?im)^Industry:\s*\S+", description):
        conn.close()
        return jsonify({"ok":False,"error":"Industry is required"}),400
    ...
    cursor=conn.execute("""INSERT INTO requirements 
        ...
        VALUES (?,?,?,?,?,?,?,?)""",
        (title,description,client_name,d.get("assigned_sourcer_id") or session.get("team_member_id"),
...
def update_requirement(rid):
    d=request.json
    conn=get_db()
    current = conn.execute("SELECT title, client_name, description, jd_url FROM requirements WHERE id=?", (rid,)).fetchone()
    if not current:
        conn.close()
    ...
    if "description" in d and ("Industry:" not in new_description or not re.search(r"(?im)^Industry:\s*\S+", new_description)):
        conn.close()
        return jsonify({"ok":False,"error":"Industry is required"}),400
...
def send_google_oauth_email(to_addr, subject, body, attachments=None):
...
def send_custom_email(to_addr, subject, body, candidate_id=None, template_name="custom", attachments=None):
    use_google = bool(session.get("google_token"))
    try:
        if use_google:
            try:
                sent_result = send_google_oauth_email(to_addr, subject, body, attachments=attachments)
                if not sent_result.get("ok"):
                    return sent_result
...
def api_daily_work_report_email():
    ...
    subject, body = build_daily_work_report_email(rows, session.get("recruiter_name") or session.get("username") or "", filters_applied)
    today_label = date.today().strftime("%d %b %Y")
    body = "\n".join([
        f"Daily Work Report - {today_label}",
        "",
        f"Sent by: {session.get('recruiter_name') or session.get('username') or '-'}",
        f"Candidates included: {len(rows)}",
        "Candidate export is attached as an Excel file.",
        "Available candidate CVs are attached separately.",
    ])
    attachments = [{
    ...
    cv_attachments = candidate_cv_attachments(rows)
    attachments.extend(cv_attachments)
    result = send_custom_email(to_addr, subject, body, template_name="daily_work_report", attachments=attachments)
